...
Depending on the speed of your connection, this could take anywhere between a
couple of seconds and a couple of minutes.
...
The reason we have thisfor this extra step is that the Add-on SDK is currently
the only Mozilla project that uses Git and GitHub as its version control and
project management tool. The rest of Mozilla uses Mercurial and Bugzilla for
these tasks. To stay consistent with the rest of Mozilla, we provide a Mercurial
mirror of our Git repository, and submit our patches for review in both GitHub
and Bugzilla.
...
to replace all instances of `YOUR_LINK_HERE`:
    ...
    <p>You can review this patch at <a href="<
        YOUR_LINK_HERE >"><YOUR_LINK_HERE></a>, or wait 5
        seconds to be redirected there automatically.</p>
...
description for the attachment, and to set the review flag to '?' The '?' here
...
* Mozilla is a global organization, with contributors all over the world, so the
  ...
  Most contributors to the SDK are current;y based in the US, so if you're in
