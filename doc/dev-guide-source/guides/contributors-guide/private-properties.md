...
Both the use of prefixes and closures has drawbacks in that they are either not
...
property names with an underscore. Consider the following example:
...
use of an underscore prefix is just a coding convention, and not enforced by the
language: there is nothing to prevent a user from directly accessing a property
...
There are two problems with the above approach. First, it is not possible to use
...
unique identifier with each image, and override it's `toString` method. The
...
The above two problems is exactly what WeakMaps were designed to solve. A
