...
const Worker = AsyncEventEmitter.compose({
  ...
  constructor: function Worker(options) {
    ...
    if (require("api-utils/xul-app").is("Fennec")) {
      ...
      else if (!Array.isArray(this.contentScriptFile))
        this.contentScriptFile = [this.contentScriptFile]
      for each (let url in this.contentScriptFile) {
        try {
          // Only ensure that this url map to a local file
          // like resource://.. urls
          // But as we use mozScriptLoader, we need to keep resource urls.
          toFilename(url);
          contentScriptFile.push(url);
        } catch(e) {
          console.exception(e);
        }
      }
