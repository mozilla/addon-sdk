...
const events = {
   ...
   */
  on: function on(element, type, listener, capture) {
   ...
   *    for a detailed explanation.
   */
  once: function once(element, type, listener, capture) {
    events.on(element, type, function selfRemovableListener(event) {
      events.removeListener(element, type, selfRemovableListener, capture);
   ...
   */
  emit: function emit(element, type, { category, initializer, settings }) {
