...
const { isWindowPrivate, getFocusedBrowser,
         browsers, windows } = require('../window/utils');
const { isPrivateBrowsingSupported } = require('../self');
const { first } = require('../util/sequence');
...
function isntCompatible(window) {
  return !isPrivateBrowsingSupported && isWindowPrivate(window) && !isGlobalPBSupported;
}
exports.ignoreWindow = isntCompatible;
exports.isntCompatible = isntCompatible;
...
// TODO: This function does not makes any sense here and we should move
// it to some place else, which would be doable without cyrcular dependencies
// once legacy private browsing code is removed.
let getActiveBrowser = () => {
  let browser = getFocusedBrowser();
  // Bug 834961: Hide private windows from addons that didn't opt-in.
  return isntCompatible(browser) ? first(browsers()) : browser;
}
