...
const Rules = Class({
  ...
  matchesAny: function(uri, nonURI) !!filterMatches(this, uri, nonURI).length,
  toString: function() '[object Rules]'
});
exports.Rules = Rules;

function filterMatches(instance, uri, nonURI) {
  let matches = [];
  for (let i in instance) {
    if (!nonURI) {
      if (new MatchPattern(instance[i], nonURI).test(uri)) {
        matches.push(instance[i]);
      }
    }
    else {
      if (new MatchPattern(instance[i], nonURI).test(uri, nonURI)) {
        matches.push(instance[i]);
      }
    }
  }
