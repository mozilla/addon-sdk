...
const Rules = Class({
  ...
  // Executes `fn` for every rule that matches `uri`
  forEachMatch: function(uri, fn) filterMatches(this, uri).forEach(fn),
  forEach: function (fn) each(this, fn)
});
exports.Rules = Rules;

function filterMatches(instance, uri) {
  let matches = [];
  each(registry(instance), function (rule, pattern) {
    if (pattern.test(uri)) matches.push(rule);
  });
  return matches;
}

function registry (instance) { return cache(instance).registry }
