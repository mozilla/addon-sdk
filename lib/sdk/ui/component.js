...
function getStateFor(component, target) {
  ...
  if (target) {
    // has a target
    if (target.tagName && target.tagName.toLowerCase() === "tab") {
      // the target is a tab
      if (states.has(target)) {
        ...
        // we do not have a tab state, try with window
  ...
  // we do not have a window state, returns component state
...
function properties(contract) {
  ...
  descriptor.state = {
    value: function state(target, state) {
      ...
      let currentState = getStateFor(this, target);
      ...
      if (state !== null)
        newState = contract(copy({}, currentState, state));

      setStateFor(this, target, newState);
      ...
      for (let window of interactiveWindows) {
        emit(this.constructor, 'render', this, window, getStateFor(this, getActiveTab(window)));
      }
    }
  }
