...
const validate = contract({
  ...
  size: {
	is: ["number"],
	ok: x => x >= 40, // That was the previously hardcoded value, honoring it
	msg: "The size must be an number greater or equal than 40"
  },
  showCloseButton: {
	is: ["boolean"],
	msg: "The options.showCloseButton must be boolean"
  }
});

// Toolbars is a mapping between `toolbar.id` & `toolbar` instances,
// which is used to find instance for dispatching events.
