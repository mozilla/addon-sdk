...
const Sidebar = Class({
  ...
  setup: function(options) {
    ...
    function updateButtonState(source, state) {
      ...
      for each (let window in wins) {
        let isShowing = isSidebarShowing(window, self);
        let newState = (source == 'button') ? state.checked : isShowing;

        // update sidebar?
        if (isShowing != newState) {
          if (newState) {
        ...
        // update the button
        setChecked(button, window, newState);
      }
    }
    let button = Button({
      id: model.id,
      ...
      onClick: updateButtonState.bind(null, 'button')
    });
    buttons.set(this, button);

    this.on('show', updateButtonState);
    this.on('hide', updateButtonState);
