...
const { properties, render, state } = require("./state");
...
let buttonType = {
  is: string,
  ok: v => isNil(v) || ~['button', 'checkbox'].indexOf(v),
...
let size = {
  is: string,
  ok: v => isNil(v) || ~['small', 'medium', 'large'].indexOf(v),
...
const Button = Class({
  ...
  // in our high level API we have `destroy`, so here an alias for consistency
