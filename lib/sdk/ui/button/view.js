...
// Hack to limit the size of the toolbarbutton's icon once in toolbar.
// It should be done properly on Firefox
...
const styleSheet = ios.newURI('data:text/css,' + buttonPrefix + '{} .toolbarbutton-icon { height: 16px }', null, null);

styleSheetService.loadAndRegisterSheet(styleSheet, styleSheetService.USER_SHEET);

require("../../system/unload").when(function() {
  styleSheetService.unregisterSheet(styleSheet);
});
// end hack
...
function create(options) {
  let { id, label, image, size } = options;
  let bus = {};
