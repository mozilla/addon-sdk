...
const buttonListener = {
  onWidgetAdded: function(widgetId, area, position) {
    ...
    if (id in views) {
      views[id].area = area;
      emit(views[id].events, 'moved', area === CustomizableUI.AREA_PANEL);
    }
  }
};
...
function getNode(id, window) {
  return ignoreWindow(window)
    ? null
    : CustomizableUI.getWidget(toWidgetID(id)).forWindow(window).node
};

function isInPanel(id) {
  return id in views && views[id].area === CustomizableUI.AREA_PANEL;
}
