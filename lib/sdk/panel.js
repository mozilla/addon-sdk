...
const { spawn, filter } = require("signalize/core")
...
let ready = filter(function({type, target}) {
  return getAttachEventType(modelFor(panelFor(target))) === type
}, panelEvents);
...
spawn(shows, function({target}) emit(panelFor(target), "show"));
spawn(hides, function({target}) emit(panelFor(target), "hide"));
spawn(ready, function({target}) {
