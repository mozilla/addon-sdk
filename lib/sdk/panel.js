...
const { contentReady, contentCreate,
        contentLoad } = require("./content/states");
...
function onReady(model, view) {
  ...
  let { contentScriptWhen } = model;
  let frame = view.frame;
  let ready = requiresAddonGlobal(model) ? contentCreate(frame) :
              contentScriptWhen === "start" ? contentCreate(frame) :
              contentScriptWhen === "end" ? contentLoad(frame) :
              contentReady(frame);
  return ready;
}
...
function attachWhenReady(instance) {
  let { view, model } = panel(instance);
  onReady(model, view).then(function onFrameReady() {
    let window = view.frame.contentWindow;
    if (!isDisposed(instance)) attach(instance, window);
  }, console.exception);
}
...
const Panel = Class({
  ...
  setup: function setup(options) {
    ...
    view.addEventListener("popupshown", this, false);
    view.addEventListener("popuphiding", this, false);
    ...
    if (isAddonContent(model) || hasContentScript(model)) {
      ...
      this.on("detach", attachWhenReady.bind(null, this));
      attachWhenReady(this);
    }
  },
  handleEvent: function handleEvent(event) {
    try {
      ...
      if (event.type === "popupshown") emit(this, "show");
      if (event.type === "popuphidden") emit(this, "hide");
    } catch(error) {
  ...
  dispose: function dispose() {
    ...
    let { view } = panel(this);
    delete panel(this).view;

    view.removeEventListener("popupshown", this, false);
    view.removeEventListener("popuphidden", this, false);
  ...
  show: function show(anchor) {
    ...
    if (!isDisposed(this) && !this.isShowing)
  ...
  hide: function hide() {
    ...
    if (this.isShowing) utils.close(view);
