...
function enable({ preferences, id }) {
  ...
  function onAddonOptionsDisplayed({ subject: doc, data }) {
    if (data === id) {
      let parent = (doc.getElementById('detail-downloads') &&
                    doc.getElementById('detail-downloads').parentNode ||
                    doc.querySelector("#addons-details > .addon-item .options-box").parentNode);
      injectOptions({
