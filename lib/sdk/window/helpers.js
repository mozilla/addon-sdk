...
function open(uri, options) {
  let newWindow = openWindow.apply(null, arguments);

  // These events arrive in different order on different platforms, only return
  // when the window is both loaded and focused.
  return all([
    promise(newWindow, 'load'),
    promise(newWindow, 'activate')
  ]).then(() => newWindow);
}
exports.open = open;
