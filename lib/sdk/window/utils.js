...
const { partial, complement } = require('../lang/functional');
const { fromEnumerator, filter } = require('../util/sequence');
...
let getWindowByOuterId = outerId => WM.getOuterWindowWithId(outerId);
exports.getWindowByOuterId = getWindowByOuterId;
...
let isntPrivate = window => !isWindowPrivate(window)
function browsers(options) {
  let { includePrivate } = options || {};
  let browsers = fromEnumerator(partial(WM.getEnumerator, BROWSER));
  return includePrivate ? browsers : filter(isntPrivate, browsers);
}
...
let isDOMWindow = window => Ci.nsIDOMWindow
exports.isDOMWindow = isDOMWindow;
...
let isFennecBrowser = window => isBrowser(window) &&
                                Boolean(window.BrowserApp);
exports.isFennecBrowser = isFennecBrowser;

let isntFennecBrowser = complement(isFennecBrowser);
let isFirefoxBrowser = window => isBrowser(window) &&
                                 !isFennecBrowser(window);
...
let getFocusedBrowser = () => WM.getMostRecentWindow(BROWSER);
...
function close(window) {
  // Close only if not a fennec, since fennec only has
  // one window.
  if (window.BrowserApp)
    throw Error('This method is not yet supported by Fennec');
...
function activate(window) {
  // Only activate window if it's not a Fennec. Fennec has
  // a single window so this method is obsolete there.
  if (!window.BrowserApp) window.focus();
}
