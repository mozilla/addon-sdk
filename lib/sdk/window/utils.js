...
function getFocusedWindow() {
  ...
  let window = browser && browser.document.commandDispatcher.focusedWindow;
  if (browser && browser === window) {
    // in e10s focus stops at the process boundary, so a workaround:
    window = browser.content;
    // it could be a (nested) iframe
    while (window.document.activeElement instanceof Ci.nsIDOMHTMLIFrameElement)
      window = window.document.activeElement.contentWindow;
  }
  return window;
}
...
function getFocusedElement() {
  ...
  let element = window && window.document.commandDispatcher.focusedElement;
  if (window && window.gBrowser.selectedBrowser.isRemoteBrowser) {
    // in e10s, focus stops at the process boundary, so a workaround
    window = getFocusedWindow();
    element = window && window.document.activeElement;
  }
  return element;
}
