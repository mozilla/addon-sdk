...
function makeHostDocument() {
  ...
  if (hiddenWindow.location.protocol == "chrome:" &&
    ...
    return resolve(hiddenWindow.document)
  }
  else {
...
var hostDocument = makeHostDocument();
...
function addHidenFrame(frame) {
  ...
  hostDocument.then(function(document) {
    let element = makeFrame(document, {
      nodeName: "iframe",
      type: "content",
      allowJavascript: true,
      allowPlugins: true,
...
unload(function () {
  ...
  hostDocument.then(function(frame) {
