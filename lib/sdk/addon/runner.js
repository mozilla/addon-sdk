...
function startup(reason, options) {
  // If the addon is droped in profile folder, it will be installed
  // during and executed during app startup. So that reason will be install.
  // We check if the session is already restored or if we need to wait.
  let sessionRestored = Cc["@mozilla.org/browser/sessionstore;1"].
                        getService(Ci.nsISessionStore).
                        restored;
  // Use strict check for sessionRestored in order to allow running
  // on firefox version without this new `nsISessionStore.restored` flag.
  if (reason === "startup" ||
      (APP_STARTUP == "sessionstore-windows-restored" &&
