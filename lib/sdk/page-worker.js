...
const Page = Class({
  ...
  setup: function Page(options) {
    let contract = pageContract(options);
    setListeners(this, options);
    let view = makeFrame(window.document, {
      nodeName: 'iframe',
      type: 'content',
      uri: contract.contentURL,
      allowJavascript: contract.allow.script,
      allowPlugins: true,
      allowAuth: true
    });
    ...
    if (options.include) {
      this.rules = Rules();
      this.rules.add.apply(this.rules, ensureArray(options.include));
    }

    // If it's local content and does not have a content script,
    // inject immediately
    if (isAddonGlobalRequired(model))
...
let pageEvents = map(events, function({target, type}) {
  return { type: type, target: target, page: pageFromDoc(target) };
});
let pageReadyEvents = filter(events, function({page, type}) {
  return page && page.contentScriptWhen === readyEventMap[type];
});
on(pageReadyEvents, 'data', function({page}) {
  emit(page, 'ready', {page: page})
});

function isReadyEvent ({type, target}) {
