...
if (!require("./system/xul-app").is("Firefox")) {
  throw new Error([
    ...
    "The selection module currently supports only Firefox."
  ].join(""));
}
...
const { Ci, Cc } = require("chrome"),
    ...
    { getTabs, getTabContentWindow } = require('./tabs/utils'),
    { getInnerId, getMostRecentBrowserWindow,
      isBrowser, windows } = require("./window/utils");
...
/**
 * Returns all tabs across all the browser's windows
 */
function getAllTabs() {
...
function getAllTabContentWindows() {
  return getAllTabs().map(getTabContentWindow);
}
 ...
 */
function context() {
  return getMostRecentBrowserWindow().document.commandDispatcher.focusedWindow;
}
...
function removeSelectionListener(window) {
  ...
  window.removeEventListener("select", selectionListener.onSelect);
