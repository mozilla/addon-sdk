  ...
  for (let key in requirements) {
    ...
    if (req.is) {
      ...
      if (Array.isArray(types)) {
        isRequired = ['undefined', 'null'].every(v => types.indexOf(v) === -1);
    ...
    if ((isRequired || !isNil(optsVal)) && req.ok && !req.ok(optsVal))
      throw new RequirementError(key, req);
...
let isTruthyType = function(type) !(type === 'undefined' || type === 'null');

function required(req) {
  req = merge({}, req);
  req.is = (Array.isArray(req.is) ? req.is : VALID_TYPES).filter(isTruthyType);
...
function optional(req) {
  ...
  req.is = req.is.filter(isTruthyType).concat('undefined', 'null');
...
function either(...types) {
  types = types.map(v => (!Array.isArray(v) && Array.isArray(v.is)) ? v.is : v);
  union.apply(null, types);
}
