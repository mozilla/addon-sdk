...
TestRunner.prototype = {
  ...
  assertNumber: function(a, message) {
    ...
    if (!this.isDone) {
      ...
      let tabs = getTabs();
      let wins = windows();
      this.assertEqual(wins.length, 1, "Should not be any unexpected windows open");
      this.assertEqual(tabs.length, 1, "Should not be any unexpected tabs open");
      if (tabs.length != 1 || wins.length != 1) {
        console.log("Windows open:");
        for (let win of wins) {
          if (isBrowser(win)) {
            tabs = getTabs(win);
            console.log(win.location + " - " + [getURI(t) for each (t in tabs)].join(", "));
          }
          else {
