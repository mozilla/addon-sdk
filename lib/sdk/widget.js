...
BrowserWindow.prototype = {
  ...
  _insertNodeInToolbar: function BW__insertNodeInToolbar(node) {
    ...
    if (!container && require("./self").loadReason == "install") {
      // Use this boolean to avoid readding the widget on new windows during
      // first run
      this._widgetAlreadyInstalled[id] = true;
    ...
    // If we do not have any toolbar here, it means that the widget
    // has been removed by the use and should be visible
    if (!container)
      return;
...
WidgetChrome.prototype.setContent = function WC_setContent() {
  // The widget might be in the toolbar.palette hidden DOM node
  // when it has been removed by the user.
  // In that case, the iframe won't be working and the DOM won't
  // be in any document tree.
  // So we listen for eventual move of the widget from the palette to
  // any toolbar in order to initialize widget's iframe.
  if (!this.node.parentNode.parentNode) {
    let self = this;
    this.node.addEventListener("ToolbaritemMove", function moveListener() {
      self.node.removeEventListener("ToolbaritemMove", moveListener);
      self.setContent();
    });
    return;
  }
