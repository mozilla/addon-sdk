...
emit.lazy = function lazy(target, type, message /*, ...*/) {
  ...
  // into a console.
  if (type === 'error' && length === 0) console.exception(message);

  // Otherwise just iterate and apply passed arguments to all listeners.
  while (index < length) {
    try { yield listeners[index].apply(target, args); }
    catch (error) {
      try { emit(target, 'error', error); }
      catch (e) {
        if (count(target, 'error') === 1) console.exception(error);
        console.exception(e);
      }
    }
    index = index + 1;
  ...
  if (type !== '*') emit.apply(emit, [target, '*', type].concat(args));
}
exports.emit = emit;
...
function setListeners(target, listeners) {
  Object.keys(listeners || {}).forEach(function onEach(key) {
    ...
    if (type && typeof(listener) === 'function') on(target, type, listener);
  });
}
