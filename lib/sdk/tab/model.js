...
getFavicon.when(isXULTab, compose(getFaviconURIForLocation, getTabURL));
getFavicon.when(isFennecTab, compose(unsupportedOnFennec,
                                      constant(BLANK_FAVICON)));

let getThumbnail = dispatcher("getThumbnail@tab");
getThumbnail.when(isXULTab, compose(getThumbnailURIForWindow,
                                     getTabContentWindow));
getThumbnail.when(isFennecTab, compose(unsupportedOnFennec,
...
modelFor.when(isXULTab, getModelByView);
...
modelFor.when(isFennecTab, getModelByView);
