...
const { events: contentEvents } = require("../content/events");
...
let readyEvents = filter(browserEvents, function(e) e.type === "DOMContentLoaded");
...
let tabContentEvents = map(contentEvents, function({target, type}) {
  return { type: type, target: getTabForContentWindow(target.defaultView) }
})
let tabStateChanges = filter(tabContentEvents, function({target}) target)

// TODO: Consider adding tab progress listeners:
// https://developer.mozilla.org/en-US/docs/Listening_to_events_on_all_tabs
