...
const WorkerSandbox = Class({
  ...
  hasListenerFor: function hasListenerFor(name) {
    let m = modelFor(this); 
    return m.hasListenerFor && m.hasListenerFor(name);
  },
  ...
  initialize: function WorkerSandbox(worker, window) {
    ...
    content.then( (global) => {
      ...
      // gc.log('zzzzzzzzzzzzzzzzzzzzz', parent);
      ...
      content.load(CONTENT_WORKER_URL).then( (ContentWorker) => {
        ...
        let result = Cu.waiveXrays(ContentWorker).inject(global, chromeAPI, onEvent, options);
  ...
  destroy: function destroy(reason) {
    if (typeof reason != 'string')
      reason = '';
    return this.emitSync('event', 'detach', reason).then(_ => {
...
function evaluateIn (workerSandbox, code, filename) {
  let { worker, sandbox } = modelFor(workerSandbox);
  sandbox.evaluate(code, filename || 'javascript:' + code).
    catch(e => emit(worker, 'error', e));
}
...
function emitToContent (workerSandbox, args) {
  let m = modelFor(workerSandbox);
  if (m.emitToContent) {
    m.emitToContent(args);
    return m.init;
  } 
  else {
    return m.init.then(_ => m.emitToContent(args));
  }
}
