...
const { requiresAddonGlobal, isAddonContent } = require('./utils');
...
const extras = require("../addon/extras");
...
const WorkerSandbox = Class({
  ...
  initialize: function WorkerSandbox(worker, window) {
    ...
    if (requiresAddonGlobal(worker)) {
      let extrasVal = extras.get();
      if (extrasVal !== undefined &&
          isAddonContent({ contentURL: window.location.href })) {
        getUnsafeWindow(window).extras = extrasVal;
      }
