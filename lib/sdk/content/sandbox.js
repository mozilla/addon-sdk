...
const { Sandbox, evaluate } = require('../loader/sandbox');
...
const WorkerSandbox = Class({
  ...
  initialize: function WorkerSandbox(worker, window) {
    ...
    let isSystemPrincipal = !Cu.isCrossProcessWrapper(window) &&
          secMan.isSystemPrincipal(window.document.nodePrincipal);
    ...
    model.init = this.init = promise;
    this.init.then(_ => this.inited = true);
    ...
    content.then( (global) => {
      ...
      try {
        let dummy = window.top;
      } catch(e) {
        console.log('WARN: window already destroyed before ContentWorker created');
        return;
      }
      // We have to ensure that window.top and window.parent are the exact same
      // object than window object, i.e. the sandbox global object. But not
      // always, in case of iframes, top and parent are another window object.
      let top = window.top === window ? global : global.top;
      let parent = window.parent === window ? global : global.parent;
      ...
      content.load(CONTENT_WORKER_URL).then( (ContentWorker) => {
        ...
        let waiveXrays = Cu.waiveXrays(ContentWorker);
        let result = waiveXrays.inject(global, chromeAPI, onEvent, options);
        ...
        if (contentScriptFile)
          promises = importScripts.apply(null, [this].concat(contentScriptFile));
        if (contentScript) {
          let p = evaluateIn(
            this,
...
function importScripts (workerSandbox, ...urls) {
  ...
  for (let contentScriptFile of urls) {
    try {
      ...
      let promise = sandbox.load(String(uri));
      promise.catch(e => emit(worker, 'error', e));
      all.push(promise);
    }
    catch(e) {
      emit(worker, 'error', e);
...
function evaluateIn (workerSandbox, code, filename) {
  let { worker, sandbox } = modelFor(workerSandbox);
  let promise = sandbox.evaluate(code, filename || 'javascript:' + code);
  promise.catch(e => emit(worker, 'error', e));
  return promise;
}
...
function onContentEvent (workerSandbox, args) {
  let { init } = modelFor(workerSandbox);
