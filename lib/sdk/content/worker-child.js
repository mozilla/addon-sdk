...
const WorkerChild = Class({
  ...
  initialize(options) {
    ...
    this.messageManager.addMessageListener('sdk/worker/message', this.receive);
  ...
  send(...args) {
    ...
    this.messageManager.sendAsyncMessage('sdk/worker/event', { id: this.id, args });
  },
  // notifications
  ...
  destroy(reason) {
    if (!this.sandbox)
      return;
    this.messageManager.removeMessageListener('sdk/worker/message', this.receive);
