...
attach.define(Worker, function (worker, window) {
  ...
  worker.init = model.contentWorker.init;

  // Mainly enable worker.port.emit to send event to the content worker
  model.contentWorker.init.then(_ => {
...
// this is my change, don't remember why i made it async..
/*
detach.define(Worker, function (worker, reason) {
  ...
  if (model.contentWorker) {
    return model.contentWorker.init.then(_ => {

      let zzx = !!model.contentWorker;
  ...
  //        console.log(e);
      ...
      if (model.windowID) {
        model.windowID = null;
        if (zzx && model.documentUnload && model.inited) events.off("inner-window-destroyed", model.documentUnload);
        model.documentUnload = null;
...
detach.define(Worker, function (worker, reason) {
  let model = modelFor(worker);
  // a bit hacky..
  if (model.contentWorker && !model.inited)
    return model.contentWorker.init.then(_ => detach(worker, reason));
...
destroy.define(Worker, function (worker, reason) {
  let model = modelFor(worker);
  if (model.contentWorker && !model.inited) 
    return model.contentWorker.init.then(_ => destroy(worker, reason));
...
// this was my change, don't remember anymore why i turned it async..
/*
destroy.define(Worker, function (worker, reason) {
  detach(worker, reason).then(_ => {
    modelFor(worker).inited = true;
    // Specifying no type or listener removes all listeners
    // from target
    off(worker);
