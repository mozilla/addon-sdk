  ...
  setup: function setup(addonPort) {
    ...
    this.on("event", function portEmit(name, args) {
      ...
      if (self._addonWorker)
        emitEventToWorker(self._addonWorker, arguments);
    });
...
const Worker = Class({
  ...
  postMessage: function postMessage(data) {
    let { model, contentWorker } = worker(this);
    if (!contentWorker) throw new Error(ERR_DESTROYED);
    if (isPaused(this.port)) throw new Error(ERR_FROZEN);

    contentWorker.emit("message", data);
  },
...
function attach(instance, view) {
  ...
  contentUnload(view).then(function() { detach(instance); });
