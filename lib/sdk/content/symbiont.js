...
const Symbiont = Class({
  ...
  _initFrame: function _initFrame(frame) {
    ...
    if ('start' == this.contentScriptWhen) {
      ...
      events.on('document-element-inserted',
  ...
  _unregisterListener: function _unregisterListener() {
    ...
    if (this._loadEvent == "start") {
      events.off('document-element-inserted', this._loadListener);
    }
    else {
   ...
   */
  _onInit: function () {
    this.setupWorker({ window: this._frame.contentWindow });
  }
});
