...
// TODO 
// This should probably be rolled up into a helper, since it's also
// used in page-mod
...
const Symbiont = Worker.resolve({
  ...
  constructor: function Symbiont(options) {
    ...
    if (this.include) {
      ...
      this.include.add.apply(null, ensureArray(include));
    }
  ...
  _validURL: function _validURL (frame) {
    let currentURL = frame.contentDocument.location.href;
