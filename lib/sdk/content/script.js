...
function Content(address, options) {
  ...
  // Another event listener is used to receives raw instead of serialized ones.
  // This is used by context-menu in order pass DOM nodes to a content-scripts.
  // Hopefully this is temporary until Bug - 824182 is fixed.
  window.document.addEventListener("☢" + contentAddress, function(event) {
    ...
    var listeners = receivers.get(message.type);
    if (!listeners || !listeners.length) return;
    listeners.slice(0).forEach(function(listener) {
      try {
        var value = listener.apply(listener, data);
...
function Chrome(address, window, port) {
  ...
  function transplant(prefix, result, type) {
    // Just like send with a difference that data is not serialized
    // and instead it's passed right to the content-script. Context
    // menu API depends on this to pass DOM nodes to the content scripts.
    // Also second argument is an array where return values of listeners
    // are pushed, that way context menu can get back results of `context`
    // event handlers. Hopefully this will go away after Bug 824182 is fixed.
    var event = window.document.createEvent("CustomEvent");
    event.initCustomEvent(prefix + contentAddress, false, true, {
      type: type,
      result: result,
      data: Array.slice(arguments, 3),
      __exposedProps__: {
        type: "r",
        result: "rw",
        data: "r"
  ...
  on(window, "*", function forward(type) {
    ...
    if (type === "☢") return transplant.apply(transplant, message);
    if (type[0] === "!") {
      message[0] = type.substr(1);
      send.apply(send, message);
    }
    else {
