...
process.port.on('sdk/contextmenu/createitems', (items) => {
...
process.port.on('sdk/contextmenu/destroyitems', (items) => {
  for (let id of items) {
...
process.port.on('sdk/contextmenu/activateitems', (items, data) => {
  for (let id of items) {
    let item = keepAlive.get(id);
    if (!item)
      continue;
