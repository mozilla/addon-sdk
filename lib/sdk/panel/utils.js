...
const { getMostRecentBrowserWindow: activeBrowser,
        getOwnerBrowserWindow: getBrowser } = require('../window/utils');
...
let hiddenDocument = appShellService.hiddenDOMWindow.document;


const XUL_NS = "http://www.mozilla.org/keymaster/gatekeeper/there.is.only.xul";
const ON_SHOW = 'popupshown';
const ON_HIDE = 'popuphidden';

function open(panel, anchor, position, x, y) {
  // Wait for the XBL binding to be constructed
  if (!panel.openPopup) setTimeout(open, 50, panel, position, x, y);
  else panel.openPopup(anchor, position, x, y);
}
exports.open = open;
...
function display(panel, width, height, anchor) {
  ...
  open(panel, anchor, position, x, y);
}
...
function show(panel, width, height, anchor) {
  ...
  display(panel, width, height, anchor);
}
exports.show = show
...
function make(document) {
  ...
  '<bindings xmlns="http://www.mozilla.org/xbl">' +
    ...
    '</binding>' +
  '</bindings>';
  panel.style.MozBinding = 'url("data:text/xml;charset=utf-8,' +
    document.defaultView.encodeURIComponent(binding) + '")';

  document.getElementById("mainPopupSet").appendChild(panel);

  let frame = createFrame(hiddenDocument, {
    allowJavascript: true,
    allowPlugins: true,
    allowAuth: true,
    uri: "data:;charset=utf-8,"
  })
  ...
  frame.addEventListener("readystatechange", console.log);
  panel.frame = frame;
...
function style(document, panel) {
  console.log("STYLE!!", document.URL)
  /**
