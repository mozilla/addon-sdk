...
const LOADING_STYLE = {
  backgroundImage: "url(\"resource:///chrome/browser/skin/classic/browser/tabbrowser/loading.png\")",
...
function make(document, options) {
  ...
  // Turn on loading styles in `viewFrame`
  toggleLoading(viewFrame);
  ...
  function onContentReady({target, type}) {
    if (target === getContentDocument(panel)) {
      // Remove loading styles from viewFrame
      toggleLoading(viewFrame);
      style(panel);
      events.emit(type, { subject: panel });
    }
...
function toggleLoading(panel) {
  // Enables loading styles if not already present
  var toggleOn = panel.style.backgroundImage !== LOADING_STYLE.backgroundImage;
  Object.keys(LOADING_STYLE).forEach(style => {
    panel.style[style] = toggleOn ? LOADING_STYLE[style] : "";
  });
}
