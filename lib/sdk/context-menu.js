...
function getDocumentForTab(tab) {
  ...
  let tabbrowser = wm.getMostRecentWindow(null).gBrowser;
  return tabbrowser.getBrowserForTab(tab).contentDocument;
}
...
let SelectionContext = Class({
  ...
  isCurrent: function isCurrent(popupNode) {
    let ownerDocument;
    if (popupNode.nodeName == "tab")
      ownerDocument = getDocumentForTab(popupNode);
    else
      ownerDocument = popupNode.ownerDocument;
...
let URLContext = Class({
  ...
  isCurrent: function isCurrent(popupNode) {
    let url;
    if (popupNode.nodeName == "tab")
      url = getDocumentForTab(popupNode).location.href;
    else
      url = popupNode.ownerDocument.URL;
...
function populateCallbackNodeData(node) {
  ...
  if (data.targetName == "tab") {
    // This is from a tab context,
    let tabData = {};
    tabData.label = node.label;
    tabData.selected = node.selected;
    data.tabData = tabData;
    let doc = getDocumentForTab(node);
    data.documentType = doc.contentType;
    data.documentURL = doc.location.href;
    data.targetID = node.id || null;
    data.isEditable = false;
  }
  else {
    // This is from a content document's context.
...
let BaseItem = Class({
  initialize: function initialize() {
    ...
    if (parentMenu) {
      ...
      contentContextMenu.addItem(this);
    }
...
let Item = Class({
  ...
  initialize: function initialize(options) {
    ...
    // If a tab context has been specified, we ignore contentScript and
    // contentScriptFile
    if (("parentMenu" in internal(this).options) &&
        (internal(this).options.parentMenu == tabContextMenu)) {
      for (let attribute of ["contentScript", "contentScriptFile"]) {
        if (attribute in internal(this).options) {
          delete internal(this).options[attribute];
        }
      }
    }
...
let tabContextMenu = ItemContainer();
...
let MenuWrapper = Class({
  ...
  updateItemVisibilities: function updateItemVisibilities(event) {
    try {
      ...
      this.setVisibility(this.items, popupNode, popupNode.nodeName == "tab" ?
                         true : PageContext().isCurrent(popupNode));
    }
    catch (e) {
      console.exception(e);
    }
...
let WindowWrapper = Class({
  initialize: function initialize(window) {
    ...
    this.menus = [
      ...
      new MenuWrapper(this, tabContextMenu, window.document.getElementById("tabContextMenu")),
    ];
  },
