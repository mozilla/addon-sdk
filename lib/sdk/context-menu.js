...
let ContextWorker = Class({
  ...
  getMatchedContext: function getCurrentContexts(popupNode) {
    ...
    return !results.length || results.reduce((val, result) => val || result);
  },
...
let MenuWrapper = Class({
  ...
  preloadWorkers: function(items, win) {
    let r = resolve(null);
    for (let item of internal(items).children) {
      let w = getItemWorkerForWindow(item, win);
      r = resolve(w && w.init).then(r);
      if (item instanceof Menu)
        r = this.preloadWorkers(item, win).then(r);
    }
    return r;
  },
  ...
  updateItemVisibilities: function updateItemVisibilities(event) {
    try {
      ...
      this.preloadWorkers(this.items, event.target.triggerNode.ownerDocument.defaultView).then(_ => {
