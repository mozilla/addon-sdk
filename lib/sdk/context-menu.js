...
let ContextWorker = Class({
  ...
  getMatchedContext: function getCurrentContexts(popupNode) {
    let results = [];
    // Note that ☢ type triggers special behavior that passes emitted input
    // without serializing it. In addition at the content script side first
    // two arguments are ignored. Second argument must be an array into
    // which return values of the listeners are collected.
    this.port.emit("☢", results, "context", popupNode);
  ...
  fireClick: function fireClick(popupNode, clickedItemData) {
    this.port.emit("☢", null, "click", popupNode, clickedItemData);
  }
});
