...
function populateCallbackNodeData(node) {
  ...
  data.documentType = null; // TODO: what value is desired here?
  ...
  if ((data.targetName === 'input' && editableInputs[node.type]) ||
      data.targetName === 'textarea') {
    data.selectionText = node.value.slice(node.selectionStart, node.selectionEnd);
  }
  else {
    data.selectionText = window.getSelection().toString();
  }
