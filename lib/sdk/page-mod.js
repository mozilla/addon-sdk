...
const PageMod = Class({
  ...
  setup: function PageMod(options) {
    ...
    if (!model.include && !model.includeType)
      console.error('Both `include` and `includeType` can not be undefined.');
...
function onContentWindow({ subject: document }) {
  ...
  for (let pagemod of pagemods) {
    // Check for the provided rulesets
...
function applyOnExistingDocuments (mod) {
  ...
  // Matches in context of tab or frames with the ruleset and returns `true`
