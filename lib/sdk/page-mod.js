...
const modOptions = {
  ...
  include: {  
    ...
    ok: (rule) => {
      ...
      if (rule.url) {
        if (isRegExpOrString(rule.url))
          return true;
        if (Array.isArray(rule.url) && rule.url.length > 0)
          return rule.url.every(isRegExpOrString);
      }
      if (rule.contentType) {
        if (isRegExpOrString(rule.contentType))
          return true;
        if (Array.isArray(rule.contentType) && rule.contentType.length > 0)
          return rule.contentType.every(isRegExpOrString);
      }
      return false;
    },
...
const PageMod = Class({
  ...
  setup: function PageMod(options) {
    ...
    console.log('After applying: ' + JSON.stringify(model.include));
...
function onContentWindow({ subject: document }) {
  ...
  for (let pagemod of pagemods) {
    if (pagemod.include.url.matchesAny(document.URL) ||
        pagemod.include.contentType.matchesAny(document.contentType)) {
...
function applyOnExistingDocuments (mod) {
  getTabs().forEach(tab => {
    ...
    if (has(mod.attachTo, "top") &&
        (mod.include.url.matchesAny(getTabURI(tab)) ||
         mod.include.contentType.matchesAny(getTabContentType(tab)))) {
      onContent(mod, window);
    }
    if (has(mod.attachTo, "frame")) {
      getFrames(window).
        filter((iframe) => (mod.include.url.matchesAny(iframe.location.href) ||
                            mod.include.contentType.matchesAny(
