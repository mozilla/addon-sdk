...
const { ensureArray } = require('./util/array');
...
// Rules cache for usage in PageMod Manager
const RULES = {};
...
const PageMod = Loader.compose(EventEmitter, {
  ...
  constructor: function PageMod(options) {
    ...
    on(rules, 'add', this._onRuleAdd = this._onRuleAdd.bind(this));
    on(rules, 'remove', this._onRuleRemove = this._onRuleRemove.bind(this));
    rules.add.apply(rules, ensureArray(include));
  ...
  destroy: function destroy() {
    ...
    this.include.forEach(this.include.remove.bind(this.include));
  ...
  _onRuleAdd: function _onRuleAdd(url) {
    // This is duplicating MatchPattern because the manager iterates
    // through all available rules amongst all page-mods
    RULES[url] = new MatchPattern(url);
