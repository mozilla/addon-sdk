...
const PageMod = Loader.compose(EventEmitter, {
  ...
  constructor: function PageMod(options) {
    ...
    if ('attachTo' in options) {
      ...
      if (!this._attachTo("top") && !this._attachTo("frame"))
  ...
  _applyOnExistingDocuments: function _applyOnExistingDocuments() {
    ...
    tabs.forEach(function (tab) {
      ...
      if (mod._attachTo("top"))
        mod._onContent(window);

      if (mod._attachTo("frame")) {
  ...
  // helper to determine if mod should be applied
  _attachTo: function(type) {
    return this.attachTo.indexOf(type) >= 0;
  },
  ...
  _onContent: function _onContent(window) {
    ...
    if (isTopDocument && !this._attachTo("top"))
      return;
    // Is a frame document and `frame` is not set, ignore
    if (!isTopDocument && !this._attachTo("frame"))
      return;
