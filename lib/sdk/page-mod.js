...
const { isBrowser, getFrames, getInnerId } = require('./window/utils');
const { getTabs, getTabContentWindow, getTabForContentWindow,
        getURI: getTabURI, getBrowserForTab } = require('./tabs/utils');
...
const PageMod = Loader.compose(EventEmitter, {
  ...
  _applyOnExistingDocuments: function _applyOnExistingDocuments() {
    ...
    function applyToTabWindow(window, url) {
      mod._onContent(window, url);
    ...
    tabs.forEach(function (tab) {
      ...
      applyToTabWindow(window, getTabURI(tab));
      if (window.history.length > 1) {
        getBrowserForTab(tab).addEventListener('pageshow', function(e) {
          if (e.persisted) 
            applyToTabWindow(e.target.defaultView);
        })
      }
    });
  },

  // url is ondly needed for special handling (like Fennec tabs)
  _onContent: function _onContent(window, url = window.location.href) {
    // not registered yet
    if (!pageModManager.has(this))
      return;

    // ensure we only try to attach once per window.
    // must use the document to actually test, as it *is* unique,
    // while the window instances can someteimes be reused (ARGH!)
    if (this._seenWindows.has(getInnerId(window))) 
      return;
    this._seenWindows.set(getInnerId(window), true);
    ...
    // centralize testing against include rules
    if (!this.include.matchesAny(url))
      return;
