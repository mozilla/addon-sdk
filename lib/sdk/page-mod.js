...
const onObsDoc = ({ objects }) => onContentWindow(objects);
ppmm.addMessageListener('sdk/obs/doc', onObsDoc);
unload(_ => ppmm.removeMessageListener('sdk/obs/doc', onObsDoc));
...
const PageMod = Class({
  ...
  setup: function PageMod(options) {
    ...
    // make sure we only apply once per window
    this.seenWindows = new WeakMap();
...
function createWorker (mod, window) {
  let worker = Worker({
    ...
    onMessage: (e) => emit(mod, 'message', e),
  });
  worker.init.then(_ => {
...
function onContent (mod, window) {
  ...
  if (mod.seenWindows.get(window))
    return;
  mod.seenWindows.set(window, true);
