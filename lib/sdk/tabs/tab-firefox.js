...
const { activateTab, getOwnerWindow, getBrowserForTab, getTabTitle, setTabTitle,
        getTabURL, setTabURL, getTabContentType, getTabId, closeTab } = require('./utils');
...
const TabTrait = Trait.compose(EventEmitter, {
  ...
  constructor: function Tab(options) {
    ...
    rawTabNS(this._tab).tab = this._public;
    tabNS(this._public).tab = this._tab;

    getPBOwnerWindow.implement(this._public, getChromeTabOwnerWindow);
...
function getChromeTabOwnerWindow(tab) {
  return getOwnerWindow(tabNS(tab).tab);
}
