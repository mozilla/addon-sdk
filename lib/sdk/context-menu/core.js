...
const isMatchingContextHandler = ({state}) =>
  isEvery(context => context.isCurrent(state.target), state.requiredContext) &&
  (count(state.optionalContext) === 0 ||
   some(context => context.isCurrent(state.target), state.optionalContext));
...
const ContextMenuExtension = Class({
  ...
  setup() {
    const listener = new WeakMessageListener(this, onMessage);
    loadModule(globalMessageManager, "framescript/context-menu", true, "onContentFrame");
    globalMessageManager.addMessageListener("sdk/context-menu/read", listener);
    globalMessageManager.addMessageListener("sdk/context-menu/readers?", listener);
  },
  ...
  [Component.initial](options={}, children) {
    ...
    return { target, children: [], readers, users, element, registry };
  },
  ...
  [onContext]({data, target}) {
    ...
    console.log("sdk/context-menu/read", data);
