...
function readURI(uri) {
  let channel = NetUtil.newChannel(uri, 'UTF-8', null);
...
function normalize(uri) {
  return uri.substr(-3) === '.js' ? uri :
         uri.substr(-5) === '.json' ? uri :
         uri + '.js';
}
...
const Require = iced(function Require(loader, requirer) {
  ...
  function require(id) {
    ...
    else if (uri.substr(-5) === '.json') {
      module = modules[uri] = Module(requirement, uri);
      module.exports = JSON.parse(readURI(uri));
      freeze(module);
    }
