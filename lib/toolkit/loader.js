...
const load = iced(function load(loader, module) {
  ...
  //  let e = new error.constructor(error.message, error.fileName, error.lineNumber);
  //  e.stk = components.stack;
  //  throw e
    //error.stk = parseStack(error.stack).filter(isntLoaderFrame)
    let stack = error.stack;
    let frames = parseStack(stack).filter(isntLoaderFrame);

    if (String(error).indexOf("opening input stream") >= 0) {
