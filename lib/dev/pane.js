...
function Debuggee(target) {
  this.target = target;
  this.form = target.form;
  this.client = target.client;
  this.id = target.form.outerWindowID;
  this.location = new URL(target.form.url);
}
Debuggee.prototype = Object.create(Source.prototype);
Debuggee.prototype.constructor = Debuggee;
Debuggee.prototype.write = function(packet) {
  let { resolve, reject, promise } = defer();
  this.client.request(packet, (response) => {
    if (response.error)
      reject(response);
    else
      resolve(response);
...
Debuggee.prototype.postMessage = function(data, origin) {
  let { resolve, reject, promise } = defer();

  this.client.request({
    to: this.form.consoleActor,
    type: "evaluateJS",
    url: module.uri,
