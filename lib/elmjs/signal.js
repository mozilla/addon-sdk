...
Reader.start = function(input) {
  Input.start(input)
  input.value = input[$source].value;
  if (input.start)
    input.start(input.value, input)
}
Reader.receive = function(input, message, source) {
  return input.next ? input.next(message, input.value, input, source) : void(0)
}
Reader.end = function(input, message, source) {
  if (input.end)
    input.end(input.value, input, source)

  return new Break()
}
Reader.error = function(input, message, source) {
  if (input.error)
    input.error(message, input, source)

  return new Break()
}
Reader.prototype = new Input()
Reader.prototype.constructor = Reader;
Reader.prototype[$start] = Reader.start
Reader.prototype[$end] = Reader.end
Reader.prototype[$receive] = Reader.receive
Reader.prototype[$error] = Reader.error
exports.Reader = Reader

function write(options, input) {
  var reader = new Reader(input, options);
  start(reader);
}
exports.write = write;
