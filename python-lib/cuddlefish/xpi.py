...
def build_xpi(template_root_dir, manifest, xpi_path, harness_options,
              locale, packages,
              root_path=None, sdk_path=None,
              limit_to=None, icon=None, icon64=None,
              extra_harness_options={}, binary=None):
    if USE_CFXJS:
        ...
        cfxjs.execute('build-xpi', {
            ## Options to allow reading FS and building the manifest in JS
            # Both are null in python tests (test_xpi.py)
            ...
            # Absolute path the the template directory
            # which mainly contains `bootstrap.js`
            'template-path': template_root_dir,
            # Content of install.rdf file
            'install-rdf': str(manifest),
            # Absolute paths to icon files to ship in the xpi
            'icon': icon,
            'icon64': icon,
            # Locale dictionnary to write in locale/{langcode}.json files
            'locale': locale,
            # Packages dictionnary with all sections absolute paths
            'packages': packages,
            # Absolute path to the xpi file to generate
            'xpi-path': xpi_path,
            # Will be mostly computed by JS side in manifest building step
            # but some attributes may still be generated by python
            # like `logFile` and `resultFile`
            'harness-options': harness_options.copy(),
            # Extra option given by the user to inject into harness-options.json
            'extra-harness-options': extra_harness_options,
            # List of absolute path of files to copy into xpi file
            # will be computed by new JS manifest implementation
            'limit-to': limit_to
        }, binary)
    ...
    if icon:
        zf.write(str(icon), 'icon.png')

    if icon64:
        zf.write(str(icon64), 'icon64.png')
    ...
    for packageName in packages:
      ...
      for sectionName in packages[packageName]:
        abs_dirname = packages[packageName][sectionName]
    ...
    mkzipdir(zf, "locale/")
    for language in sorted(locale):
        locales_json_data["locales"].append(language)
        locale_dict = locale[language]
        # Be carefull about strings, we need to always ensure working with UTF-8
        jsonStr = json.dumps(locale_dict, indent=1, sort_keys=True, ensure_ascii=False)
