...
class ManifestBuilder:
    def __init__(self, target_cfg, pkg_cfg, deps, extra_modules,
                 stderr=sys.stderr, stop_on_missing=False):
        ...
        self.stop_on_missing = stop_on_missing # cfx eol
            ...
            if reqname == "chrome" or reqname.startswith("@"):
                ...
                if them_me is None:
                    if mi.section == "tests":
                        ...
                        continue
                    if not self.stop_on_missing:
                        # tolerate missing modules unless --stop-on-missing
                        # cfx argument was used, but also print a warning
...
def build_manifest(target_cfg, pkg_cfg, deps, scan_tests,
                   test_filter_re=None, extra_modules=[], stop_on_missing=False):
    """
