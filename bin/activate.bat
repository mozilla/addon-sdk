...
for /f "tokens=* delims= " %%A in ('echo %PYTHONINSTALL%') do set PYTHONINSTALL=%%A
...
for /f "tokens=* delims= " %%A in ('echo %PYTHONINSTALL%') do set PYTHONINSTALL=%%A
if exist "%PYTHONINSTALL%\python.exe" goto :EOF
if exist "%PYTHONINSTALL%\PCBuild\python." (set "PYTHONINSTALL=%PYTHONINSTALL%\PCBuild" & goto :EOF)
if exist "%PYTHONINSTALL%\PCBuild\amd64\python.exe" (set "PYTHONINSTALL=%PYTHONINSTALL%\PCBuild\amd64" & goto :EOF)
